...
//	memcpy(ccm + 0xc00, buffer + 0x6000, 0x1400); // Display and Frequency Data 5k

	uint8_t *myProgramImage = buffer, *bankPtr = buffer + 0x5000;
	uint8_t *myDisplayImage = buffer + 0x6000, *myFrequencyImage = buffer + 0x7000;
			        	...
			        	// callFunction(value);
			        	  ...
			        	  switch (data_prev)
			        	  {
			        	    ...
			        	    case 1: // Copy ROM to fetcher
			        	    	//DMA2_Stream0->
			        	    	myDataFetcherCopyPointer = myParameter[3];			        	    	myDataFetcherCopyType = (uint8_t) data_prev;
