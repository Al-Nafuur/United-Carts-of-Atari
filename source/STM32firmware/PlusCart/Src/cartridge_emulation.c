		...
		{ // A12 high
			if(withPlusFunctions && addr > 0x1fef && addr < 0x1ff4){
				if(addr == 0x1ff2 ){// read from receive buffer
					...
					out_buffer[out_buffer_write_pointer] = (uint8_t) data_prev;
				...
				{	// SC RAM access
					...
					{	// a read from cartridge ram
						DATA_OUT = ((uint16_t)cart_ram[addr&0x7F]);
						SET_DATA_MODE_OUT
						// wait for address bus to change
						while (ADDR_IN == addr) ;
						SET_DATA_MODE_IN
					...
					{	// a write to cartridge ram
						...
						while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
						cart_ram[addr&0x7F] = (uint8_t) data_prev;
					}
				}
				else
				{	// normal rom access
	...
	while (1)
	{
		...
		{ // A12 high
			if(withPlusFunctions && addr > 0x1fef && addr < 0x1ff4){
				...
				{	// a read from cartridge ram
					DATA_OUT = ((uint16_t)cart_ram[addr&0xFF]);
					SET_DATA_MODE_OUT
					// wait for address bus to change
					while (ADDR_IN == addr) ;
					SET_DATA_MODE_IN
				...
				{	// a write to cartridge ram
					// read last data on the bus before the address lines change
					while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
					cart_ram[addr&0xFF] = (uint8_t) data_prev;
				}
				else
				{	// normal rom access
					DATA_OUT = ((uint16_t)bankPtr[addr&0xFFF]);
	...
	while (1)
	{
		...
		{ // A12 high
			if (addr & 0x800)
				...
				DATA_OUT = ((uint16_t)bankPtr[addr&0x7FF]);
			SET_DATA_MODE_OUT
			// wait for address bus to change
			while (ADDR_IN == addr) ;
			SET_DATA_MODE_IN
		...
		{ // A12 high
			if(withPlusFunctions && addr > 0x1fef && addr < 0x1ff4){
				...
				{	// we are accessing the RAM write addresses ($1400-$17FF)
					// read last data on the bus before the address lines change
					while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
					bankPtr[addr&0x3FF] = (uint8_t) data_prev;
				}else
		...
		{ // A12 high
			if(withPlusFunctions && addr > 0x1fef && addr < 0x1ff4){
				...
				{	// we are accessing a RAM write address ($1200-$13FF, $1600-$17FF, $1A00-$1BFF or $1E00-$1FFF)
					// read last data on the bus before the address lines change
					while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
					bankPtr[act_bank][addr & 0x1FF] = (uint8_t) data_prev;
				}
				else
...
void emulate_E0_cartridge()
{
	...
	while (1)
	{
		...
		{ // A12 high
			...
			DATA_OUT = ((uint16_t)cart_rom[curBanks[target]*1024 + (addr&0x3FF)]);
			SET_DATA_MODE_OUT
			// wait for address bus to change
			while (ADDR_IN == addr) ;
			SET_DATA_MODE_IN
		...
		{ // A12 high
			...
			{	// RAM access
				...
				{	// a write to cartridge ram
					// read last data on the bus before the address lines change
					while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
					cart_ram[addr&0x3FF] = (uint8_t) data_prev;
				}
				else
				{	// a read from cartridge ram
					DATA_OUT = ((uint16_t)cart_ram[addr&0x3FF]);
					SET_DATA_MODE_OUT
					// wait for address bus to change
					while (ADDR_IN == addr) ;
					SET_DATA_MODE_IN
...
void emulate_F0_cartridge()
{
	...
	while (1)
	{
		...
		{ // A12 high
			...
			// ROM access
			DATA_OUT = ((uint16_t)cart_rom[(currentBank * 4096)+(addr&0xFFF)]);
			SET_DATA_MODE_OUT
			// wait for address bus to change
			while (ADDR_IN == addr) ;
			SET_DATA_MODE_IN
			...
			{	// higher 2k cartridge ROM area
				...
				{	// 256 byte RAM access
					...
					{	// 1900-19FF is the read port
						DATA_OUT = ((uint16_t)ram1Ptr[addr&0xFF]);
						SET_DATA_MODE_OUT
						// wait for address bus to change
						while (ADDR_IN == addr) ;
						SET_DATA_MODE_IN
					...
					{	// 1800-18FF is the write port
						while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
						ram1Ptr[addr&0xFF] = (unsigned char) data_prev;
					}
				}
				else
			...
			{	// lower 2k cartridge ROM area
				...
				{	// 1K RAM access
					...
					{	// 1400-17FF is the read port
						DATA_OUT = ((uint16_t)ram2Ptr[addr&0x3FF]);
						SET_DATA_MODE_OUT
						// wait for address bus to change
						while (ADDR_IN == addr) ;
						SET_DATA_MODE_IN
					...
					{	// 1000-13FF is the write port
						while (ADDR_IN == addr) { data_prev = data; data = DATA_IN; }
						ram2Ptr[addr&0x3FF] = (unsigned char) data_prev;
					}
				}
				else
	...
	while (1)
	{
		...
		{ // A12 high
			...
			{	// DPC write
				...
				unsigned char value = (unsigned char) data_prev;
				switch (function)
				{
				case 0x00:
				{	// DFx top count
	...
	while (1) {
		...
		if (addr & 0x1000) {
			...
			if (caddr < 0x40) {
				...
				ram[caddr - 0x40] = (uint8_t) data_prev;
			}
			else {
