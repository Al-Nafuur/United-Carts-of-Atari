...
void emulate_dfsc_cartridge(const char* filename, uint32_t image_size, uint8_t* buffer) {
    ...
    for (uint8_t i = 0; i < CCM_DF_BANKS; i++) layout->banks[RAM_BANKS + i] = CCM_RAM + i * 4096;
...
void emulate_bfsc_cartridge(uint32_t flash_part_address ) {
    uint8_t *ram_base = buffer + AVAILABLE_RAM_BASE;
    ...
    for (uint8_t i = 0; i < RAM_BANKS; i++) layout->banks[i] = buffer + i * 4096;
    for (uint8_t i = 0; i < CCM_BF_BANKS; i++) layout->banks[RAM_BANKS + i] = CCM_RAM + i * 4096;
    for (uint8_t i = 0; i < FLASH_BANKS; i++) layout->banks[RAM_BANKS + CCM_BF_BANKS + i] = ((uint8_t*)(flash_part_address)) + i * 4096;
...
void emulate_bf_cartridge(uint32_t flash_part_address ) {
    uint8_t *ram_base = buffer + AVAILABLE_RAM_BASE;
    cartridge_layout* layout = (void*)ram_base;
