...
#define BUFFER_SIZE_BYTES (96*1024)
...
int launch_ace_cartridge( const char* filename, uint32_t image_size, uint8_t* buffer, MENU_ENTRY *d,int header_length, bool withPlusFunctions, uint32_t* CCMPTR)
{
	...
	if(d->type == Cart_File ){
		if(ace_type == UnoOnly)
			return 0;
#if USE_WIFI
		// Copy what was already downloaded to flash
		...
		flash_buffer_at(buffer, buffered_size, cart_rom);
		// Then download the rest if any
		if(unbuffered_size > 0) {
			flash_download_at((char*)filename, unbuffered_size, buffered_size, cart_rom + buffered_size);
		}
		...
		if(ace_type == PlusOnly){
			...
			while(bytes_remaining > 0){
				...
				uint32_t bytes_read = sd_card_file_request(buffer, filename, start_pos, length);
				if(bytes_read == length){
					flash_buffer_at(buffer, bytes_read, cart_rom+start_pos);
				} else {
					return 0;
				}
	...
	*buffer32 = (uint32_t)cart_rom; //0. Declare the start position of the cart in flash as a 32 bit pointer. This changes depending on the state of the user's offline roms.
	buffer32++;
	*buffer32 = (uint32_t)(CCMPTR);//1. Declare the usage of CCM Memory so ACE application can use remainder, and not disturb the Pluscart variables.
	buffer32++;
	*buffer32 = (uint32_t)(&reboot_into_cartridge); //2. Pass Pluscart library function pointer for reboot_into_cartridge. Used for bootstrapping the 2600 system before running the ACE application.
	buffer32++;
	*buffer32 = (uint32_t)(&exit_cartridge); //3. Pass Pluscart library function pointer for emulate_firmware_cartridge. Used to exit out of a 2600 rom and back into the Pluscart menu.
	buffer32++;
