...
START	SEI                     ; Disable interrupts
	...
	TXS                     ; Init Stack
	LDA  #$00
        STA RequestPending ; This is not clean, we only need 
                           ; RequestPending to be zero at startup.
...
NoStir  LDX  #$01              ; Start with P1
	...
	LDA  MY_SWCHA          ; Joysticks. Before we return via
...
RdTrig  BIT  GameOn             ; Branch if no game on
	...
	LDA  MY_INPT4,X         ; Read Input (Trigger) X.
...
; PAL
;	byte $2A ,$4C ,$D2 ,$64      ; 00 = Regular Tanks
;	.byte $42 ,$2C ,$DA ,$3A      ; 01 = Tank Pong
;	.byte $D0 ,$BC ,$3A ,$4A      ; 10 = Jets
;	.byte $A4 ,$98 ,$3A ,$6A      ; 11 = Biplanes
; B&W
...
; PlusROM stuff..
APICheck				; check server response
	LDA #0
	CMP ReceiveBufferSize		; byte(s) recieved ?
	BEQ CheckSendRequest		; no -> Check to Send Request
	STA RequestPending		; enable requesting again
	LDA ReceiveBuffer		; yes load (first) received byte (MY_SWCHA)
        STA MY_SWCHA
	LDA ReceiveBuffer		; load (2.) received byte (MY_INPT4)
        STA MY_INPT4
...
CheckSendRequest
	...
	BNE APIEnd			; no -> Send new Request
...
PlusROMAPI:				; PlusROM API url
	...
	org $FFF0			; Hotspots and detection for
        				; PlusROM enabled emulator/device
	.byte 0				; WriteToBuffer
	.byte 0				; WriteSendBuffer
	.byte 255			; ReceiveBuffer
	.byte 255			; ReceiveBufferSize

;************************************************************   
; Special memory locations. Tells the 6502 where to go.
	org $FFFA
	.word $1f00    ; PlusROMAPI
	.word $f000
	.word $f000
