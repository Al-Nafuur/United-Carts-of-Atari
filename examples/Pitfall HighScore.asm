...
; 6/2017:
; + fix SCREENSAVER = 0
; - replace LFSR (must stay 8 bits!)
; - use different seeds and EOR values
...
TRAINER         = 0                     ; enable training mode
 ELSE ;---------------------------------------------------------------------
OPTIMIZE        = 1                     ; enable some possible optimizations
FILL_OPT        = 0                     ; fill optimized bytes with NOPs
SCREENSAVER     = 0                     ; compile with screensaver code
SINGLE_GAME     = 1
TRAINER         = 0                     ; enable training mode
 ENDIF ;/ORIGINAL
...
RAND_SEED       = $c4                   ; defines the start scene of the game
  ELSE
GAME_IDX        = 1
;NEXT_RND    = $b4       ; %10110100
;PREV_RND    = <(NEXT_RND*2+1) ;$69       ; %01101001
NUM_GAMES       = 16
  ENDIF
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Thomas Jentzsch's free space macros
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
.doQuickSand:
  IF SINGLE_GAME
    ...
    lda    frameCnt         ; 3
  ELSE
; fix for stopped game display:
    lda    #$80             ;
    ldx    noGameScroll     ; 3                 game running?
    bne    .stopFrames      ; 2³                 no, skip
    lda    frameCnt         ; 3
.stopFrames
  ENDIF
    lsr                     ; 2
...
; calculate x-positioning values:
  ...
  IF SCREENSAVER
    lda    #0               ; 2
    ldy    SS_Delay         ; 3                 screen saver enabled?
    bmi    .skipHarryPos    ; 2³                 yes, don't draw wall and scorpion
  ELSE
    FILL_NOP 6
  ENDIF
.loopPos:
...
SkipHarry5:
  ...
  IF SCREENSAVER | FILL_OPT
    nop                     ; 2
  ENDIF
    beq    ContHarry5       ; 3+1               page crossed!
;    CHECKPAGEFAULT ContHarry5
...
; Harry is loosing a live:
 ...
 IF PLUSROM
    bne    .decreaseLives   ; 2³                yes, decrease
...
.noScreenSaver:
    ...
    sta    SS_Mask          ; 3
  ELSE
    FILL_NOP 39-10          ;                   -10 avoids extra line
  ENDIF
...
ColorTab:
  IF NTSC_COL | ORIGINAL
    .byte $0C                       ; score and copyright color
  ELSE
    .byte $0C+2                     ; score and copyright color
  ENDIF
    .byte $0C                       ; unused
...
GroundTypeTab:
 ...
 IF PLUSROM    
    ORG_FREE_LBL $fffa, "Startup Vector"
    .word (PlusROM_API-$E000)
 ELSE
    ORG_FREE_LBL $fffc, "Startup Vector"
 ENDIF

    .word $f000
    .word 0

    ECHO "Free:", [FREE_TOTAL]d, "bytes"
