...
; - Assembly switch NO_WINDMILLS added to easily build game without windmills
; - Assembly switch NO_GEESE added to easily build game without geese
...
COMPILE_VERSION         = NTSC      ; change to compile for different regions
   ...
   IF !(COMPILE_VERSION = NTSC || COMPILE_VERSION = PAL50 || COMPILE_VERSION = PAL60)
   ...
   IFNCONST NO_WINDMILLS
   
NO_WINDMILLS            = FALSE     ; builds with Windmills

   ENDIF
   
   IF !(NO_WINDMILLS = TRUE || NO_WINDMILLS = FALSE)

      echo ""
      echo "*** ERROR: Invalid NO_WINDMILLS value"
      echo "*** Valid values: NO_WINDMILLS = 1 for no Windmills, NO_WINDMILLS = 0 for Windmills"
      echo ""
      err
...
NO_GEESE                = FALSE     ; builds with Geese

   ENDIF
   
   IF !(NO_GEESE = TRUE || NO_GEESE = FALSE)

      echo ""
      echo "*** ERROR: Invalid NO_GEESE value"
      echo "*** Valid values: NO_GEESE = 1 for no Geese, NO_GEESE = 0 for Geese"
      echo ""
      err
...
SPAWN_TOP_GOOSE         = %000
SPAWN_MIDDLE_GOOSE      = %000
SPAWN_BOTTOM_GOOSE      = %000
...
SendPlusROMScore:
    ...
    lda elapsedTime+3
    sta WriteToBuffer
    lda #9                  	     ; Barnstorming game id in Highscore DB
